...
public partial class Operations : ContentPage
{
    ...
    private async void AddServiceCategory_Clicked(object sender, EventArgs e)
    {
        ...
        {
            await DisplayAlert("Service Category", "Enter the full service category name before saving.", "OK");
            return;
        }
    ...
    private async void DeleteServiceCategory_Clicked(object sender, EventArgs e)
    {
        ...
        {
            await DisplayAlert("Service Category", "Select a category to delete.", "OK");
            return;
        }
        ...
        {
            await DisplayAlert("Service Category", "The selected category could not be found.", "OK");
            return;
        }

        if (ServiceItems.Any(service => service.CategoryId == selectedCategory.CategoryId && service.IsActive))
        {
            await DisplayAlert("Service Category", "This category is currently used by services and cannot be deleted.", "OK");
            return;
        }

        var confirm = await DisplayAlert("Delete Category", $"Delete {selectedCategory.CategoryName}?", "Delete", "Cancel");
        if (!confirm)
        {
            return;
        }
        ...
        try
        {
            ...
            await DisplayAlert("Service Category", "Service category deleted.", "OK");
        }
        catch (Exception ex)
        {
            await DisplayAlert("Service Category", ex.Message, "OK");
    ...
    private async Task<UpsertAdminShopService?> BuildServiceRequestAsync()
    {
        ...
        {
            await DisplayAlert("Missing Details", "Please enter a service category, service name, base price, and estimated minutes.", "OK");
            return null;
        }
        ...
        if (category is null)
        {
            ...
            {
                await DisplayAlert("Service Category", $"Unable to create {categoryName}: {ex.Message}", "OK");
                return null;
            }
        }
