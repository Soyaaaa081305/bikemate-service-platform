...
public partial class AccCreate2 : ContentPage
{
    ...
    private async void OnPickValidIdClicked(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!scan.IsSuccessful)
            {
                ...
                await DisplayAlert("ID Scan", scan.ErrorMessage ?? "The ID scan could not be completed.", "OK");
                return;
            }
            ...
            {
                await DisplayAlert("ID Scan", "BikeMate could not prepare the scanned ID image. Please scan again.", "OK");
                return;
            }
    ...
    private async void OnScanValidIdClicked(object sender, EventArgs e)
    {
        ...
        if (!result.IsSuccessful)
        {
            ...
            await DisplayAlert("ID Scan", result.ErrorMessage ?? "The ID scan could not be completed.", "OK");
            return;
        }
        ...
        await DisplayAlert("ID Scan Completed", "This preview was processed on your device only. Use Upload to scan and submit the ID for BikeMate admin approval.", "OK");
    }
    ...
    private async void OnContinueClicked(object sender, EventArgs e)
    {
            ...
            string.IsNullOrWhiteSpace(_draft.ValidIdPath))
        {
            ...
            await DisplayAlert("Missing Information", message, "OK");
            return;
        }
