...
public sealed class ServicesController(BikeMateDbContext db) : ControllerBase
{
    ...
    {
        var category = await db.ServiceCategories.SingleOrDefaultAsync(x => x.CategoryId == categoryId, cancellationToken);
        if (category is null || !category.IsActive)
        {
            return NotFound(new { message = "Service category was not found or has already been deleted." });
        }

        var isUsedByActiveServices = await db.ShopServices.AnyAsync(
            x => x.CategoryId == categoryId && x.IsActive,
            cancellationToken);
        if (isUsedByActiveServices)
        {
            return BadRequest(new { message = "This category is currently used by services and cannot be deleted." });
        }
        ...
        [FromQuery] string? concern = null)
    {
        ...
        return Ok(shops
            .Where(shop => shop.Services.Any(service => service.IsActive))
            .OrderByDescending(shop => shop.Services.Any(service =>
                service.IsActive &&
