...
internal static class BookingDraft
{
    ...
    public const string AssistanceOnSite = "On-site Repair";
    public const string AssistancePickup = "Pick-up Repair";
    public const string AssistanceReservation = "Reservation";
    public const string TimingReservation = "Scheduled reservation";
    ...
    public static List<int> SelectedShopServiceIds { get; } = [];
    ...
    public static List<ProductDto> SelectedProducts { get; } = [];
    ...
    public static string AssistanceTiming { get; set; } = "";
    ...
    public static string IssueSummary()
    {
        ...
        var product = SelectedProducts.Count > 0
        ...
        var timing = string.IsNullOrWhiteSpace(AssistanceMethod)
            ? ""
            : $"\nTiming: {ScheduleSummary()}";
        return $"Booking type: {workType}\n{concern}{product}\nVehicle: {VehicleSummary()}{method}{timing}";
    }

    public static bool IsReservation =>
        string.Equals(AssistanceMethod, AssistanceReservation, StringComparison.OrdinalIgnoreCase) ||
        string.Equals(AssistanceTiming, TimingReservation, StringComparison.OrdinalIgnoreCase);

    public static string ScheduleSummary()
    {
        return FormatSchedule(null);
    }

    public static string FormatSchedule(DateTime? scheduledAt)
    {
        if (scheduledAt is not null)
        {
            return scheduledAt.Value.ToLocalTime().ToString("MMM d, yyyy 'at' h:mm tt", CultureInfo.InvariantCulture);
        }

        return IsReservation
            ? ScheduledAt.ToString("MMM d, yyyy 'at' h:mm tt", CultureInfo.InvariantCulture)
            : string.Equals(AssistanceMethod, AssistancePickup, StringComparison.OrdinalIgnoreCase)
                ? "Shop pickup after confirmation"
                : "At-home repair after shop confirmation";
    }

    public static string AssistanceLocationLabel()
    {
        return IsReservation
            ? "Shop visit area"
            : string.Equals(AssistanceMethod, AssistancePickup, StringComparison.OrdinalIgnoreCase)
                ? "Pickup address"
                : "At-home repair address";
    }
    ...
    public static ShopServiceDto? SelectedService()
    {
        ...
        if (explicitService is not null && explicitService.IsActive)
        {
    ...
    public static IReadOnlyList<ShopServiceDto> SelectedServices()
    {
        var selected = Services
            .Where(x => SelectedShopServiceIds.Contains(x.ShopServiceId) && x.IsActive)
            .OrderBy(x => SelectedShopServiceIds.IndexOf(x.ShopServiceId))
            .ToArray();
        ...
        var fallback = SelectedService();
        return fallback is null ? [] : [fallback];
    }

    public static string SelectedServicesSummary()
    {
        var names = SelectedServices().Select(x => x.ServiceName).Where(x => !string.IsNullOrWhiteSpace(x)).ToArray();
        return names.Length == 0 ? "Choose services" : string.Join(", ", names);
    }

    public static string SelectedProductsSummary()
    {
        return SelectedProducts.Count == 0
            ? "No products selected"
            : string.Join(", ", SelectedProducts.Select(x => $"{x.ProductName} ({BookingVisuals.Money(x.Price)})"));
    }

    public static decimal SelectedItemsTotal()
    {
        return SelectedServices().Sum(x => x.BasePrice) + SelectedProducts.Sum(x => x.Price);
    }
    ...
    public static void SelectShop(ShopSummaryDto shop)
    {
    ...
    {
        return !string.IsNullOrWhiteSpace(requestShopName)
            ? requestShopName
            : !string.IsNullOrWhiteSpace(SelectedShopName)
                ? SelectedShopName
                : "Selected BikeMate shop";
    }

    public static string? DisplayShopImageUrl(ServiceRequestDto? request = null)
    {
        return request?.ShopLogoUrl ?? request?.ShopImageUrl ?? SelectedShopLogoUrl ?? SelectedShopImageUrl;
    }

    public static void SelectProduct(ProductDto product)
    {
        if (SelectedProducts.Any(x => x.ProductId == product.ProductId))
        {
            SelectedProducts.RemoveAll(x => x.ProductId == product.ProductId);
        }
        else
        {
        ...
        var primary = SelectedProducts.FirstOrDefault();
        SelectedProductId = primary?.ProductId;
        SelectedProductName = primary?.ProductName ?? "";
        SelectedProductPrice = primary?.Price;
    }
    ...
    public static void ToggleService(ShopServiceDto service)
    {
        if (SelectedShopServiceIds.Contains(service.ShopServiceId))
        {
            SelectedShopServiceIds.Remove(service.ShopServiceId);
        }
        else
        {
            SelectedShopServiceIds.Add(service.ShopServiceId);
        }

        SelectedShopServiceId = SelectedShopServiceIds.FirstOrDefault();
        if (SelectedShopServiceId == 0)
        {
            SelectedShopServiceId = null;
        }
    }
...
internal static class BookingFlowActions
{
    public static async Task<ServiceRequestDto> EnsureRequestAsync(bool attachMedia)
    {
        ...
        {
            throw new InvalidOperationException(CustomerRequestRules.BookingBlockedMessage(customer));
        }
        ...
        var shopServiceIds = shopId is null ? Array.Empty<int>() : BookingDraft.SelectedShopServiceIds.ToArray();
        var shopServiceId = shopId is null ? null : BookingDraft.SelectedShopServiceId;
        if (shopId is not null && shopServiceIds.Length == 0 && shopServiceId is null)
        {
            ...
            if (shopServiceId is not null)
            {
                BookingDraft.SelectedShopServiceIds.Add(shopServiceId.Value);
                shopServiceIds = [shopServiceId.Value];
            }
        }
        ...
        if (BookingDraft.RequestId == 0)
        {
            request = await CustomerApiClient.CreateRequestAsync(new CreateServiceRequestDto(
                ...
                BookingDraft.IsReservation ? BookingDraft.ScheduledAt.ToUniversalTime() : null,
                shopServiceIds,
                BookingDraft.SelectedProducts.Select(x => x.ProductId).ToArray()));
    ...
    {
        if (BookingDraft.SelectedShopServiceIds.Count == 0 && shopServiceId is not null)
        {
            BookingDraft.SelectedShopServiceIds.Add(shopServiceId.Value);
        }

        if (BookingDraft.SelectedShopServiceIds.Count == 0)
        {
            throw new InvalidOperationException("Choose at least one exact shop service before continuing to payment.");
        }

        BookingDraft.SelectedShopId = shopId;
        BookingDraft.SelectedShopServiceId = BookingDraft.SelectedShopServiceIds.FirstOrDefault();
        var request = await EnsureRequestAsync(true);
        var selected = await CustomerApiClient.SelectShopAsync(
            request.RequestId,
            new SelectShopDto(
                shopId,
                BookingDraft.SelectedShopServiceId,
                BookingDraft.SelectedProductId,
                BookingDraft.SelectedShopServiceIds.ToArray(),
                BookingDraft.SelectedProducts.Select(x => x.ProductId).ToArray()));
        if (!string.IsNullOrWhiteSpace(selected.ShopName))
        {
            BookingDraft.SelectedShopName = selected.ShopName;
            BookingDraft.SelectedShopImageUrl = selected.ShopImageUrl;
            BookingDraft.SelectedShopLogoUrl = selected.ShopLogoUrl;
        }

        await CustomerPageBase.ScheduleBookingRemindersAsync(new[] { selected });
...
internal static class BookingVisuals
{
    ...
    {
        return PublicOrAssetImageSource(url, ShopImage);
    }

    public static ImageSource MechanicImageSource(string? url)
    {
        return PublicOrAssetImageSource(url, MechanicImage);
    }
    ...
    public static ImageSource AssetImageSource(string asset)
    {
        return CustomerUi.Image(asset);
    }

    private static ImageSource PublicOrAssetImageSource(string? url, string fallbackAsset)
    {
        return CustomerUi.PublicOrAssetImage(url, fallbackAsset);
    }

    public static View ShopProfileCard(
        string? shopName,
        string? shopImageUrl,
        string? shopLogoUrl,
        string? subtitle = null,
        string title = "Repair shop")
    {
        var displayName = string.IsNullOrWhiteSpace(shopName)
            ? "Selected BikeMate shop"
            : shopName.Trim();
        var grid = new Grid
        {
        ...
        grid.Add(new Border
        {
            ...
            Content = new Image
            {
                Source = ShopImageSource(shopLogoUrl ?? shopImageUrl),
                WidthRequest = 62,
                HeightRequest = 62,
        ...
        copy.Add(Text(title.ToUpperInvariant(), 9, CustomerUi.Muted, FontAttributes.Bold));
        copy.Add(Text(displayName, 14, CustomerUi.Dark, FontAttributes.Bold));
        copy.Add(Text(
            string.IsNullOrWhiteSpace(subtitle) ? "Verified BikeMate repair partner" : subtitle,
            10,
            CustomerUi.Muted));
        grid.Add(copy, 1, 0);
        return WhiteCard(grid, 8, new Thickness(12));
    }
...
"""
    ...
    public static View StepIntro(
        ...
        int totalSteps = 6)
    {
        var displayedStep = totalSteps == 7 && !BookingDraft.IsReservation && step > 3 ? step - 1 : step;
        var displayedTotal = totalSteps == 7 && !BookingDraft.IsReservation ? 6 : totalSteps;
        ...
        for (var index = 0; index < displayedTotal; index++)
        {
            ...
            progress.Add(new BoxView
            {
                HeightRequest = 4,
                CornerRadius = 2,
                Color = index < displayedStep ? CustomerUi.Orange : Color.FromArgb("#DEDEDE")
            }, index, 0);
        }

        stack.Add(progress);
        stack.Add(Text($"Step {displayedStep} of {displayedTotal}  |  {stepLabel}", 10, CustomerUi.Muted, FontAttributes.Bold));
        return stack;
    }
            ...
            actions.Add(new Button
            {
                ...
                Command = new Command(async () =>
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        if (page is not null)
                        {
                            Debug.WriteLine($"Location lookup failed: {ex}");
                            await page.DisplayAlertAsync("Location", "Location could not be completed. Check permissions and try again.", "OK");
                        }
                    }
                })
            }, 0, 0);
        ...
        Command command)
    {
        ...
        header.Add(new Border
        {
            ...
            Stroke = Colors.Transparent,
            StrokeShape = new RoundRectangle { CornerRadius = 8 },
            BackgroundColor = uploaded ? Color.FromArgb("#E8F6EF") : Color.FromArgb("#FFF3EA"),
            Content = new Label
            {
                Text = mediaLabel,
    ...
    public static string FriendlyError(string message, Exception ex)
    {
...
internal sealed class LocationAccessPage : ContentPage
{
    ...
    private async Task AccessLocationAsync()
    {
        ...
        {
            Debug.WriteLine($"Location access failed: {ex}");
            await DisplayAlertAsync("Location unavailable", "Location could not be completed. Check permissions and try again.", "OK");
        }
        finally
        {
            _isBusy = false;
...
public sealed class BookingFillUpPage : CustomerPageBase
{
    ...
    protected override async void OnAppearing()
    {
        ...
        try
        {
            BookingDraft.Customer = await EnsureCustomerApprovedForBookingAsync(BookingDraft.Customer);
        ...
        {
            Render(BookingVisuals.FriendlyError("Booking details could not be prepared. Check your connection and try again.", ex));
        }
    }
    ...
    private void Render(string? banner = null)
    {
        ...
        if (BookingDraft.IsModification)
        {
            ...
            body.Add(BookingVisuals.Text(
                "Describe the upgrade you want. You will choose the shop item and installation service after the assistance step.",
                10,
                CustomerUi.Muted));
        }
        else
    ...
    private static Border SelectableCard(View content, bool selected)
    {
        return new Border
        {
            BackgroundColor = selected ? Color.FromArgb("#FFF3EA") : Colors.White,
            Stroke = selected ? CustomerUi.Orange : CustomerUi.Border,
...
public sealed class BookingServiceTypePage : CustomerPageBase
{
    ...
    private void Render()
    {
        ...
        body.Add(BookingVisuals.StepIntro(
            "How should this service happen?",
            "Choose the service flow that matches where the repair will happen.",
            3,
            "Assistance method",
            7));
        ...
        body.Add(ServiceTypeCard(
            BookingDraft.AssistanceOnSite,
            "Repair at your house or selected service address.",
            "Flat tires, brake adjustments, chain issues, and quick diagnostics.",
            "The mechanic goes to your saved home/service location after the shop confirms.",
            recommended == BookingDraft.AssistanceOnSite));
        body.Add(ServiceTypeCard(
            BookingDraft.AssistancePickup,
            "The shop picks up the vehicle for workshop service.",
            "Major repairs, detailed inspection, tune-ups, and longer service work.",
            "The partner shop coordinates pickup and return after payment confirmation.",
            recommended == BookingDraft.AssistancePickup));
        body.Add(ServiceTypeCard(
            BookingDraft.AssistanceReservation,
            "Visit the repair shop at a reserved date and time.",
            "Planned repairs, tune-ups, inspections, or modifications.",
            "You go to the selected shop for your appointment slot.",
            recommended == BookingDraft.AssistanceReservation));
        ...
        body.Add(BookingVisuals.PrimaryButton(
            BookingDraft.IsReservation ? "Continue to reservation schedule" : "Continue",
        ...
        bool recommended)
    {
        ...
        card.GestureRecognizers.Add(new TapGestureRecognizer
        {
            Command = new Command(() =>
            {
                ...
                BookingDraft.AssistanceTiming = string.Equals(title, BookingDraft.AssistanceReservation, StringComparison.OrdinalIgnoreCase)
                    ? BookingDraft.TimingReservation
                    : "";
                if (BookingDraft.IsReservation)
                {
    ...
    private async Task ContinueAsync()
    {
        if (string.IsNullOrWhiteSpace(BookingDraft.AssistanceMethod))
        {
            await DisplayAlertAsync(
                ...
                "Select on-site repair, pick-up repair, or reservation before continuing.",
                "OK");
            return;
        }
    ...
    private static string RecommendedAssistanceMethod()
    {
        if (BookingDraft.IsModification)
        {
            return BookingDraft.AssistanceReservation;
        }

        return BookingDraft.ProblemCategory switch
        {
            "Tire Problem" or "Brake Adjustment" or "Chain Maintenance" => BookingDraft.AssistanceOnSite,
            _ => BookingDraft.AssistancePickup
        };
    }
}
...
public sealed class BookingSchedulePage : CustomerPageBase
{
    ...
    private void Render()
    {
        ...
        body.Add(BookingVisuals.FlowHeader("Reservation schedule"));
        body.Add(BookingVisuals.StepIntro(
    ...
    private static View SelectedScheduleCard()
    {
        ...
        var copy = new VerticalStackLayout { Spacing = 3 };
        copy.Add(BookingVisuals.Text("Selected reservation", 10, CustomerUi.Muted));
...
public sealed class BookingUploadPage : CustomerPageBase
{
    ...
    private async Task UploadAsync(string mediaType)
    {
        ...
        {
            Render(BookingVisuals.FriendlyError("Upload failed. Check the file and try again.", ex));
        }
    }
...
public sealed class BookingConfirmationPage : CustomerPageBase
{
    ...
    protected override async void OnAppearing()
    {
        ...
        {
            Render(BookingVisuals.FriendlyError("Confirmation details could not be loaded. Check your connection and try again.", ex));
        }
    }
    ...
    private void Render(string? banner = null)
    {
        ...
        body.Add(BookingVisuals.StepIntro(
            ...
            BookingDraft.IsModification
                ? "Review the service flow, vehicle, timing, and modification notes before BikeMate finds nearby shops."
                : "Review the service flow, vehicle, timing, and concern before BikeMate finds nearby shops.",
            6,
            "Review",
            7));
        if (!string.IsNullOrWhiteSpace(banner))
        ...
        var appointmentRows = new List<View>
        {
            DetailRow("Assistance method", BookingDraft.AssistanceMethod),
            DetailRow("Timing", BookingDraft.ScheduleSummary())
        };
        if (BookingDraft.IsReservation)
        {
            appointmentRows.Add(DetailRow("Scheduled date", schedule.ToString("MMM d, yyyy", CultureInfo.InvariantCulture)));
            appointmentRows.Add(DetailRow("Scheduled time", schedule.ToString("h:mm tt", CultureInfo.InvariantCulture)));
        }
        appointmentRows.Add(DetailRow("Attachments", mediaCount == 0 ? "No media attached" : $"{mediaCount} file{(mediaCount == 1 ? "" : "s")} attached"));
        body.Add(SectionCard("Appointment details", appointmentRows));
    ...
    private static View MapSummaryCard()
    {
        ...
        bottom.Add(BookingVisuals.Text(BookingDraft.AssistanceLocationLabel(), 12, CustomerUi.Dark, FontAttributes.Bold));
    ...
    private static View PriceCard(ShopServiceDto? service)
    {
        ...
        var amount = BookingDraft.SelectedItemsTotal();
        ...
        header.Add(Pill(hasShop ? "Selected" : "Next step", hasShop ? Color.FromArgb("#E8F6EF") : Color.FromArgb("#FFF3EA"), hasShop ? Color.FromArgb("#147A3D") : CustomerUi.Orange), 1, 0);
        stack.Add(header);
        ...
        if (BookingDraft.SelectedProducts.Count > 0 || BookingDraft.IsModification)
        {
            stack.Add(DetailRow("Selected items", BookingDraft.SelectedProducts.Count == 0 ? "Optional" : BookingDraft.SelectedProductsSummary()));
        }
        stack.Add(DetailRow("Exact services", hasShop ? BookingDraft.SelectedServicesSummary() : "Choose after selecting a shop"));
...
public sealed class StoreSelectionPage : CustomerPageBase
{
    ...
    private IReadOnlyList<ShopSummaryDto> _otherShops = [];
    ...
    protected override async void OnAppearing()
    {
        ...
        try
        {
            ...
            _shops = (await shopsTask)
                ...
                .ToArray();
            _nearbyShops = _shops
                .Where(IsNearbyOrSameArea)
                .OrderBy(shop => ShopDistance(shop) ?? decimal.MaxValue)
        ...
        {
            _isLoading = false;
            Render(BookingVisuals.FriendlyError("Nearby shops could not be loaded. Check your connection and try again.", ex));
        }
    }
    ...
    private void Render(string? banner = null)
    {
        ...
        else if (_shops.Count == 0)
        {
            ...
            empty.Add(BookingVisuals.Text(
                BookingDraft.IsModification
                ? $"No verified BikeMate partner within 50 km of {BookingDraft.LocationName} currently lists active services. Try another service area or check again after shops add services."
                    : $"No verified BikeMate partner within 50 km of {BookingDraft.LocationName} currently lists active services. Try another service area or repair concern.",
                11,
                CustomerUi.Muted));
        ...
        else
        {
            body.Add(SectionHeader(
                BookingDraft.IsModification ? "Nearby modification shops" : "Nearby repair shops"));
            if (_nearbyShops.Count == 0)
            {
                body.Add(BookingVisuals.WhiteCard(
                    BookingVisuals.Text(
                        BookingDraft.Latitude is null || BookingDraft.Longitude is null
                            ? "BikeMate does not have mapped coordinates for your service area yet, so all available shops are listed below."
                            : $"No mapped shop is within {NearbyRadiusKm:N0} km of your service area. Other verified shops are listed below.",
                        11,
                        CustomerUi.Muted),
                    8,
                    new Thickness(12)));
            ...
            body.Add(SectionHeader(
                BookingDraft.IsModification ? "Other available shops" : "Other shops outside your range"));
            if (_otherShops.Count == 0)
            {
                body.Add(BookingVisuals.WhiteCard(
                    BookingVisuals.Text("Every available shop is already shown above.", 11, CustomerUi.Muted),
                    8,
                    new Thickness(12)));
            }
            else
            {
                foreach (var shop in _otherShops)
                {
                    body.Add(StoreCard(shop));
                }
            }
    ...
    private View StoreCard(ShopSummaryDto shop)
    {
        var service = _services
            ...
            .OrderByDescending(x => BookingDraft.IsRelevantService(x))
    ...
    private static string DistanceLine(ShopSummaryDto shop, decimal? distance)
    {
        if (distance is not null)
        {
            return $"{distance:0.0} km away | Verified partner";
        }
    ...
    {
        var distance = ShopDistance(shop);
        return distance is not null
            ? distance <= NearbyRadiusKm
            : IsSameServiceArea(shop);
    }

    private static bool IsSameServiceArea(ShopSummaryDto shop)
    {
        var serviceArea = string.Join(" ", new[]
            {
                BookingDraft.LocationName,
                BookingDraft.AddressLine,
                BookingDraft.Province
            })
            .Trim();
        ...
        return ContainsArea(serviceArea, shop.City) ||
            ContainsArea(serviceArea, shop.AddressLine) ||
            ContainsArea(shop.City, BookingDraft.LocationName) ||
            ContainsArea(shop.AddressLine, BookingDraft.LocationName);
    }

    private static bool ContainsArea(string? source, string? value)
    {
        ...
        var normalizedSource = NormalizeArea(source);
        var normalizedValue = NormalizeArea(value);
        return normalizedSource.Contains(normalizedValue, StringComparison.OrdinalIgnoreCase) ||
            normalizedValue.Contains(normalizedSource, StringComparison.OrdinalIgnoreCase);
    }
    ...
    private async Task BookHereAsync(ShopSummaryDto? shop)
    {
        if (shop is null)
        {
...
public sealed class StoreDetailsPage : CustomerPageBase, IQueryAttributable
{
    ...
    private string? _optionalDataMessage;
    ...
    protected override async void OnAppearing()
    {
        ...
        try
        {
            ...
            if (_shopId > 0)
            {
                _optionalDataMessage = null;
                _shop = await CustomerApiClient.GetShopDetailsAsync(_shopId);
                BookingDraft.SelectShop(_shop);
                _services = (await CustomerApiClient.GetShopServicesAsync(_shopId))
                    .Where(service => service.IsActive)
                    .OrderByDescending(service => BookingDraft.IsRelevantService(service))
                    .ThenBy(x => x.BasePrice)
                    .ToArray();
                _products = await LoadProductsOptionalAsync(_shopId);
                _reputation = await LoadReputationOptionalAsync(_shopId);
                BookingDraft.SelectedShopServiceIds.RemoveAll(id => _services.All(x => x.ShopServiceId != id));
                BookingDraft.SelectedShopServiceId = BookingDraft.SelectedShopServiceIds.FirstOrDefault();
                if (BookingDraft.SelectedShopServiceId == 0)
                {
                    BookingDraft.SelectedShopServiceId = null;
                }

                BookingDraft.SelectedProducts.RemoveAll(product => _products.All(x => x.ProductId != product.ProductId));
                if (BookingDraft.SelectedProducts.Count == 0)
                {
                    BookingDraft.ClearProduct();
                }
                else
                {
                    var primary = BookingDraft.SelectedProducts.First();
                    BookingDraft.SelectedProductId = primary.ProductId;
                    BookingDraft.SelectedProductName = primary.ProductName;
                    BookingDraft.SelectedProductPrice = primary.Price;
                }
            }
            Render();
        }
        ...
        {
            Render(BookingVisuals.FriendlyError("Shop details could not be loaded. Check your connection and try again.", ex));
        }
    }
    ...
    private void Render(string? banner = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(_optionalDataMessage))
        {
            body.Add(BookingVisuals.WhiteCard(BookingVisuals.Text(_optionalDataMessage, 10, CustomerUi.Muted), 8, new Thickness(12)));
        }
        ...
        body.Add(SectionHeading(
            ...
            BookingDraft.IsModification
                ? "Choose the labor service that matches the item or custom work."
                : "Choose the closest available service from this shop."));
        ...
        body.Add(SectionHeading(
            BookingDraft.IsModification ? "Optional items" : "Products and parts",
            BookingDraft.IsModification
                ? "Choose any shop items this modification needs. Service-only custom work can continue without products."
                : "Choose any customer-visible parts needed for this repair."));
        if (_products.Count == 0)
        {
            body.Add(BookingVisuals.WhiteCard(
                BookingVisuals.Text(
                    BookingDraft.IsModification
                        ? "No public products are listed for this shop yet. You can still continue with a service-only modification."
    ...
    private async Task<IReadOnlyList<ProductDto>> LoadProductsOptionalAsync(int shopId)
    {
        try
        {
            return (await CustomerApiClient.GetShopProductsAsync(shopId))
                .Where(x => x.IsActive)
        ...
        {
            Debug.WriteLine($"Shop products failed for shop {shopId}: {ex}");
            _optionalDataMessage = "Some shop products could not be loaded. You can still continue with a repair service.";
            return [];
        }
    }

    private async Task<ShopReputationDto?> LoadReputationOptionalAsync(int shopId)
    {
        try
        {
            return await CustomerApiClient.GetShopReputationAsync(shopId);
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"Shop reputation failed for shop {shopId}: {ex}");
            _optionalDataMessage = string.IsNullOrWhiteSpace(_optionalDataMessage)
                ? "Some shop details could not be loaded. You can still continue with an available service."
                : _optionalDataMessage;
            return null;
        }
    }
    ...
    private static View ShopReviewCard(ShopCustomerReviewDto review)
    {
        ...
        header.Add(ReviewProviderImages(review), 0, 0);
        ...
        reviewer.Add(BookingVisuals.Text(
            string.Join(" | ", new[]
            {
                ...
                string.IsNullOrWhiteSpace(review.ServiceName) ? null : review.ServiceName
            }.Where(value => !string.IsNullOrWhiteSpace(value))),
            9,
            CustomerUi.Muted));
        reviewer.Add(BookingVisuals.Text($"Technician: {review.TechnicianName}", 9, CustomerUi.Muted));
        header.Add(reviewer, 1, 0);
        header.Add(new Border
        {
            Stroke = Colors.Transparent,
            ...
            Content = BookingVisuals.Text($"{review.Rating}.0", 11, CustomerUi.Orange, FontAttributes.Bold)
        }, 2, 0);
        stack.Add(header);
        ...
        if (!string.IsNullOrWhiteSpace(review.Comment))
        {
            stack.Add(new Border
            {
                ...
                Content = BookingVisuals.Text(review.Comment, 11, CustomerUi.Dark)
            });
        }
    ...
    private static View ReviewProviderImages(ShopCustomerReviewDto review)
    {
        ...
        var shopImage = ReviewAvatar(BookingVisuals.ShopImageSource(review.ShopLogoUrl ?? review.ShopImageUrl), 42, 8);
        AbsoluteLayout.SetLayoutBounds(shopImage, new Rect(0, 0, 42, 42));
        layout.Add(shopImage);

        var mechanicImage = ReviewAvatar(BookingVisuals.MechanicImageSource(review.TechnicianProfileImageUrl), 28, 14);
        AbsoluteLayout.SetLayoutBounds(mechanicImage, new Rect(30, 14, 28, 28));
        layout.Add(mechanicImage);

        return layout;
    }

    private static Border ReviewAvatar(ImageSource source, double size, double radius)
    {
        return new Border
        {
            WidthRequest = size,
    ...
    private View ProductCard(ProductDto product)
    {
        ...
        description = CleanRepeatedDescription(description, product.ProductName, category);
        var selected = BookingDraft.SelectedProducts.Any(x => x.ProductId == product.ProductId);
        var canSelect = product.StockQuantity > 0;
        ...
        copy.Add(BookingVisuals.Text(
            ...
            FontAttributes.Bold));
        grid.Add(copy, 2, 0);
        var action = new VerticalStackLayout { Spacing = 6, HorizontalOptions = LayoutOptions.End };
        var price = BookingVisuals.Text(BookingVisuals.Money(product.Price), 12, CustomerUi.Orange, FontAttributes.Bold);
        price.HorizontalTextAlignment = TextAlignment.End;
        action.Add(price);
        action.Add(ChoiceButton(selected ? "Added" : "Add", selectCommand, canSelect, selected));
        grid.Add(action, 3, 0);
    ...
    {
        var selected = BookingDraft.SelectedShopServiceIds.Contains(service.ShopServiceId);
        var selectCommand = new Command(() =>
        {
            BookingDraft.ToggleService(service);
            Render();
        });
        var grid = new Grid
        {
        ...
        var serviceDescription = CleanRepeatedDescription(service.ServiceDescription, service.ServiceName, service.CategoryName);
        if (!string.IsNullOrWhiteSpace(serviceDescription))
        {
            text.Add(BookingVisuals.Text(serviceDescription, 11, CustomerUi.Muted));
        }

        grid.Add(text, 1, 0);
        var action = new VerticalStackLayout { Spacing = 6, HorizontalOptions = LayoutOptions.End };
        var price = BookingVisuals.Text(BookingVisuals.Money(service.BasePrice), 13, CustomerUi.Orange, FontAttributes.Bold);
        price.HorizontalTextAlignment = TextAlignment.End;
        action.Add(price);
        action.Add(ChoiceButton(selected ? "Added" : "Add", selectCommand, true, selected));
        grid.Add(action, 2, 0);
        var card = BookingVisuals.WhiteCard(grid, 8, new Thickness(12));
        card.BackgroundColor = selected ? Color.FromArgb("#FFF7F2") : Colors.White;
        card.Stroke = selected ? CustomerUi.Orange : CustomerUi.Border;
    ...
    private static Button ChoiceButton(string text, ICommand command, bool enabled, bool selected)
    {
        return new Button
        {
            Text = text,
            ...
            IsEnabled = enabled,
            BackgroundColor = selected ? CustomerUi.Orange : Colors.White,
            TextColor = selected ? Colors.White : CustomerUi.Orange,
            BorderColor = CustomerUi.Orange,
            BorderWidth = 1,
            CornerRadius = 8,
            HeightRequest = 34,
            MinimumWidthRequest = 84,
    ...
    private static string CleanRepeatedDescription(string? value, params string?[] knownLabels)
    {
        if (string.IsNullOrWhiteSpace(value))
        {
            return string.Empty;
        ...
        var lines = value
            ...
            .Where(line =>
                ...
                !line.StartsWith("Product category:", StringComparison.OrdinalIgnoreCase) &&
                known.All(label => !string.Equals(line, label, StringComparison.OrdinalIgnoreCase)))
    ...
    private async Task ContinueToPaymentAsync()
    {
        ...
        try
        {
            ...
            if (BookingDraft.SelectedShopServiceIds.Count == 0)
            {
                await DisplayAlertAsync("Choose a service", "Select at least one available service before payment.", "OK");
                return;
            }
...
public sealed class TaskerProfilePage : CustomerPageBase
{
    ...
    protected override async void OnAppearing()
    {
        ...
        {
            Render(BookingVisuals.FriendlyError("Technician profile could not be loaded. Check your connection and try again.", ex));
        }
    }
        ...
        var profile = new VerticalStackLayout
        {
            ...
            Children =
            {
                new Border
                {
                    ...
                    Content = new Image
                    {
                        Source = BookingVisuals.MechanicImageSource(_mechanic?.ProfileImageUrl),
                        Aspect = Aspect.AspectFill
                    }
                },
...
public sealed class BookingTrackMapPage : CustomerPageBase
{
    ...
    protected override async void OnAppearing()
    {
        ...
        {
            Render(BookingVisuals.FriendlyError("Tracking details could not be loaded. Check your connection and try again.", ex));
        }
    }
    ...
    private void Render(string? banner = null)
    {
        ...
        card.Add(RouteLine("Timing", BookingDraft.FormatSchedule(_request?.ScheduledAt)));
        card.Add(RouteLine(
            "Booked on",
            _request is null
...
public sealed class TrackOrderPage : CustomerPageBase
{
    ...
    private async Task LoadAsync()
    {
        ...
        {
            _banner = BookingVisuals.FriendlyError("BikeMate could not load the latest booking status. Check your connection and try again.", ex);
            Render(_banner);
        }
    }
    ...
    private void Render(string? banner = null)
    {
        ...
        var serviceName = request is null ? BookingDraft.SelectedServicesSummary() : RequestServiceTitle(request);
        ...
        if (request is not null || !string.IsNullOrWhiteSpace(BookingDraft.SelectedShopName))
        {
            body.Add(BookingVisuals.ShopProfileCard(
                BookingDraft.DisplayShopName(request?.ShopName),
                request?.ShopImageUrl ?? BookingDraft.SelectedShopImageUrl,
                request?.ShopLogoUrl ?? BookingDraft.SelectedShopLogoUrl,
                serviceName));
        }
        body.Add(SectionCard(
            "Appointment",
            [
                ...
                DetailRow(BookingDraft.AssistanceLocationLabel(), request?.ServiceLocationAddress ?? BookingDraft.ConfirmationAddress())
            ]));
        body.Add(SectionCard(
            "Repair provider",
            [
                ...
                DetailRow("Services", serviceName),
                DetailRow("Products", request is null ? BookingDraft.SelectedProductsSummary() : RequestProductsTitle(request)),
